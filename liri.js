...
function retrieveTweets() {
  ...
  fs.appendFile(
    "./log.txt",
    "User Command: node liri.js my-tweets\n\n",
    err => {
      if (err) throw err;
    }
  );
  ...
  var params = {screen_name: "WakandanJediOG", count: 20};
...
function spotifySong(song) {
  ...
  spotify.search({type: "track", query: searchStr}, function(error, data) {
    if (error) {
      ...
      if (!songInfo) {
        var errorStr2 =
          "ERROR: No song info retrieved, please check the spelling of the song name!";
  ...
  request(queryStr, function(error, response, body) {
    if (error || response.statusCode !== 200) {
      ...
      if (!data.Title && !data.Released && !data.imdbRating) {
        var errorStr2 =
          "ERROR: No movie info retrieved, please check the spelling of the movie name!";
...
// justDoIt will read in a file to determine the desired command and then execute
function justDoIt() {
  // Append the command to the log file
  fs.appendFile(
    "./log.txt",
